#include "DOSTestClient_SurfaceView.h"
...
static void ovrGeometry_CreateCube(ovrGeometry* geometry) {
    ...
    static const ovrCubeVertices cubeVertices = {
        // positions
        {
            {(char)-127, +127, (char)-127, +127},
            {+127, +127, (char)-127, +127},
            {+127, +127, +127, +127},
            {(char)-127, +127, +127, +127}, // top
            {(char)-127, (char)-127, (char)-127, +127},
            {(char)-127, (char)-127, +127, +127},
            {+127, (char)-127, +127, +127},
            {+127, (char)-127, (char)-127, +127} // bottom
        },
        // colors
        {{255, 0, 255, 255},
         {0, 255, 0, 255},
...
public:
    ...
    ovrUniform::index m_index;
    ovrUniform::type m_type;
    const char* m_pszName = nullptr;
};

static ovrUniform ProgramUniforms[] = {
    {ovrUniform::index::UNIFORM_MODEL_MATRIX,
     ovrUniform::type::UNIFORM_TYPE_MATRIX4X4,
     "ModelMatrix"},

    {ovrUniform::index::UNIFORM_VIEW_ID,
     ovrUniform::type::UNIFORM_TYPE_INT,
     "ViewID"},

    {ovrUniform::index::UNIFORM_SCENE_MATRICES,
     ovrUniform::type::UNIFORM_TYPE_BUFFER,
     "SceneMatrices"},
};
...
static bool ovrProgram_Create(
    ...
    for (int i = 0; i < (int)(sizeof(ProgramUniforms) / sizeof(ProgramUniforms[0])); i++) {
        const int uniformIndex = ProgramUniforms[i].m_index;
        if (ProgramUniforms[i].m_type == ovrUniform::type::UNIFORM_TYPE_BUFFER) {
            GL(program->UniformLocation[uniformIndex] =
                   glGetUniformBlockIndex(program->Program, ProgramUniforms[i].m_pszName));
            ...
            GL(program->UniformLocation[uniformIndex] =
                   glGetUniformLocation(program->Program, ProgramUniforms[i].m_pszName));
...
static ovrLayerProjection2 ovrRenderer_RenderFrame(
    ...
    for (int eye = 0; eye < renderer->NumBuffers; eye++) {
        ...
        GL(glBindBufferBase(
            GL_UNIFORM_BUFFER,
            scene->Program.UniformBinding[ovrUniform::index::UNIFORM_SCENE_MATRICES],
            scene->SceneMatrices));
        if (scene->Program.UniformLocation[ovrUniform::index::UNIFORM_VIEW_ID] >=
            0) // NOTE: will not be present when multiview path is enabled.
        {
            GL(glUniform1i(scene->Program.UniformLocation[ovrUniform::index::UNIFORM_VIEW_ID], eye));
        }
