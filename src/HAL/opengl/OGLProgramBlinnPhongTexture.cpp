...
RESULT OGLProgramBlinnPhongTexture::OGLInitialize(version versionOGL) {
	RESULT r = R_PASS;
	...
	// Vertex
	CRM(MakeVertexShader(L"blinnPhongTexture.vert"), "Failed to create vertex shader");

	// Fragment
	CRM(MakeFragmentShader(L"blinnPhongTexture.frag"), "Failed to create fragment shader");
