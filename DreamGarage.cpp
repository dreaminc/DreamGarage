...
//  initial positioning of buttons
	...
	m_pUserInterface = AddComposite();

	for (int i = 0; i < m_numButtons; i++) {
		std::shared_ptr<composite> pButton = m_pUserInterface->AddComposite();
		std::shared_ptr<quad> q = pButton->AddQuad(1.0f, 1.0f);

		///*
		FlatContext *pContext = AddFlatContext(1024, 1024);
...
RESULT DreamGarage::SendHandPosition() {
	...
	if (pRightHand != nullptr) {
		...
		OVERLAY_DEBUG_SET("orientUpdate2", pRightHand->GetOrientation());
	}
...
RESULT DreamGarage::Update(void) {
	...
	if (m_UIDirty) {
		...
		quaternion q = GetCamera()->GetOrientation();

		vector v = q.RotateVector(vector::kVector(1.0f));
		vector v2 = vector(v.x(), 0.0f, v.z());
		v2.Normalize();

		float degz = asin(v2.z());
		float sign = degz > 0 ? -1.0f : 1.0f;
		float degx = acos(v2.x());
		float degx2 = (sign * degx + (float)M_PI_2) * 180.0f / M_PI;
		m_rotationY = degx2;
		OVERLAY_DEBUG_SET("deg2", degx2);

		quaternion q2 = quaternion::MakeQuaternionWithEuler(0.0f, sign * degx + (float)M_PI_2, 0.0f);

		m_pUserInterface->SetPosition(GetCamera()->GetPosition());
		m_pUserInterface->SetOrientation(q2);
	}

	// Update UI	
	hand *pRightHand = GetHand(hand::HAND_TYPE::HAND_RIGHT);
	if (pRightHand != nullptr) {
		quaternion q = pRightHand->GetOrientation();
		...
		float eulerY = sign * degx + (float)M_PI_2;
		float eulerYDeg = eulerY * 180.0f / M_PI;
	//	eulerYDeg += 20.0f;
		OVERLAY_DEBUG_SET("deg1", eulerYDeg - m_rotationY);
		int shift = m_numButtons / 2;
		m_rotationY += m_rotationY < 0.0f ? 360.0f : 0.0f;
		eulerYDeg += eulerYDeg < 0.0f ? 360.0f : 0.0f;
		int index = int((eulerYDeg - m_rotationY) / 20.0f + shift + shift + 0.5f);
		OVERLAY_DEBUG_SET("index", index);
		
		if (m_selectedIndex != index) {
			// swap enlarged thing
			if (0 <= index && index < m_numButtons) {
				auto p = m_pUserInterface->GetChildren()[m_numButtons - 1 - index];
			...
			if (0 <= m_selectedIndex && m_selectedIndex < m_numButtons) {
				auto p = m_pUserInterface->GetChildren()[m_numButtons - 1 - m_selectedIndex];
				std::shared_ptr<composite> pButton = std::dynamic_pointer_cast<composite>(p);
				if (pButton != nullptr) {
					auto q = pButton->GetChildren()[0];
					q->ScaleX(m_UIScale.x());
