...
#define ACR(res) do{if(r&0x80000000){assert(0); }}while(0);   // TODO: work on this
...
#define CBRM(condition, failCode, msg, ...) do{if(!condition) { DEBUG_MSG(CurrentFileLine); DEBUG_MSG(msg, __VA_ARGS__); DEBUG_MSG("\n"); r = failCode; goto Error; }}while(0);

#define ACB(condition) do{if(!condition){assert(0);}}while(0);  // TODO: Make this work
...
#define CNM(pointer, msg, ...) do{if(pointer == NULL) { DEBUG_MSG(CurrentFileLine); DEBUG_MSG(msg, __VA_ARGS__); DEBUG_MSG("\n"); r = R_ERROR; goto Error; }}while(0);
#define CNRM(pointer, failCode, msg, ...) do{if(pointer == NULL) { DEBUG_MSG(CurrentFileLine); DEBUG_MSG(msg, __VA_ARGS__); DEBUG_MSG("\n"); r = failCode; goto Error; }}while(0);
