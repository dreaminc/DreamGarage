...
//
void D3D11DesktopDuplicationDisplayManager::InitD3D(DX_RESOURCES* Data) {
	m_pDevice = Data->Device;
	m_pDeviceContext = Data->Context;
	m_pVertexShader = Data->VertexShader;
	m_pPixelShader = Data->PixelShader;
	m_pInputLayout = Data->InputLayout;
	m_pSamplerLinear = Data->SamplerLinear;
...
//
DUPL_RETURN D3D11DesktopDuplicationDisplayManager::ProcessFrame(_In_ FRAME_DATA* Data, _Inout_ ID3D11Texture2D* SharedSurf, INT OffsetX, INT OffsetY, _In_ DXGI_OUTPUT_DESC* DeskDesc) {
...
//
void D3D11DesktopDuplicationDisplayManager::SetMoveRect(_Out_ RECT* SrcRect, _Out_ RECT* DestRect, _In_ DXGI_OUTPUT_DESC* DeskDesc, _In_ DXGI_OUTDUPL_MOVE_RECT* MoveRect, INT TexWidth, INT TexHeight) {
...
DUPL_RETURN D3D11DesktopDuplicationDisplayManager::CopyMove(_Inout_ ID3D11Texture2D* SharedSurf, _In_reads_(MoveCount) DXGI_OUTDUPL_MOVE_RECT* MoveBuffer, UINT MoveCount, INT OffsetX, INT OffsetY, _In_ DXGI_OUTPUT_DESC* DeskDesc, INT TexWidth, INT TexHeight) {
	...
	for (UINT i = 0; i < MoveCount; ++i) {
		...
		Box.back = 1;
		m_pDeviceContext->CopySubresourceRegion(m_pMoveSurf, 0, SrcRect.left, SrcRect.top, 0, SharedSurf, 0, &Box);
...
void D3D11DesktopDuplicationDisplayManager::SetDirtyVert(_Out_writes_(NUMVERTICES) VERTEX* Vertices, _In_ RECT* Dirty, INT OffsetX, INT OffsetY, _In_ DXGI_OUTPUT_DESC* DeskDesc, _In_ D3D11_TEXTURE2D_DESC* FullDesc, _In_ D3D11_TEXTURE2D_DESC* ThisDesc) {
...
DUPL_RETURN D3D11DesktopDuplicationDisplayManager::CopyDirty(_In_ ID3D11Texture2D* SrcSurface, _Inout_ ID3D11Texture2D* SharedSurf, _In_reads_(DirtyCount) RECT* DirtyBuffer, UINT DirtyCount, INT OffsetX, INT OffsetY, _In_ DXGI_OUTPUT_DESC* DeskDesc) {
	...
	// Fill them in
	VERTEX* DirtyVertex = reinterpret_cast<VERTEX*>(m_pDirtyVertexBufferAlloc);
