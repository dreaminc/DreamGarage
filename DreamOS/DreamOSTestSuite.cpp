...
RESULT DreamOSTestSuite::AddTestSettings() {
	RESULT r = R_PASS;
	...
	auto fnInitialize = [&](void *pContext) {
		...
		pTestContext->pDreamUserApp = m_pDreamOS->LaunchDreamApp<DreamUserApp>(this, false);
		CN(pTestContext->pDreamUserApp);

		pTestContext->pDreamUIBar = m_pDreamOS->LaunchDreamApp<DreamUIBar>(this, false);
		CN(pTestContext->pDreamUIBar);
	...
	auto fnUpdate = [&](void *pContext) {
		...
		pTestContext->pDreamUserApp->ResetAppComposite();
		pTestContext->pDreamUIBar->ResetAppComposite();
	...
	auto pSettingsTest = AddTest(fnInitialize, fnUpdate, fnTest, fnReset, pTestContext);
	CN(pSettingsTest);

	pSettingsTest->SetTestName("2D camera test");
	pSettingsTest->SetTestDescription("Basic test moving camera with gamepad");
	pSettingsTest->SetTestDuration(sTestTime);
	pSettingsTest->SetTestRepeats(nRepeats);
