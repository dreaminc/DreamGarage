...
void main(void) {  
	...
	if(texture(u_textureDepth, DataIn.vertShadowCoordinate.xy).x  <  DataIn.vertShadowCoordinate.z){
		lightVisibility = 0.5;
	}
	...
	out_vec4Color = max((vec4LightValue * DataIn.color * lightVisibility), ambientColor);
}