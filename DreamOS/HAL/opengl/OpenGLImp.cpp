...
RESULT OpenGLImp::Render(SceneGraph *pSceneGraph) {
	...
	OGLProfiler::Render(this, m_pOGLOverlayProgram);
...
//#define OVERLAY_ON
	...
	{
		pOGLTrianlge = new OGLTriangle(this, triangle(point(-0.5, -0.5, 0), point(0.5, -0.5, 0), point(0.5, 0.5, 0)));
	...
	if (pFPS == nullptr)
	{
		...
		pFPS = new OGLText(this, pFont, "000");
	}

	{
		...
		for (;;)
		{
			...
			if (cnt == 0)
			{
				...
				m_pOGLOverlayProgram->RenderObject(pFPS);
			}
			...
			m_pOGLOverlayProgram->RenderObject(pOGLTrianlge);
	...
	if (pOGLText == nullptr)
	{
		...
		pOGLText = new OGLText(this, pFont, "Dream Garage");

		pOGLText->MoveTo(-1.0, -1.0, 0);
	}

	m_pOGLOverlayProgram->RenderObject(pOGLText);
