...
RESULT OGLProgramGBuffer::OGLInitialize() {
	RESULT r = R_PASS;
	...
	//// Normals
	//CR(m_pOGLFramebuffer->AddColorAttachment());
	//CR(m_pOGLFramebuffer->GetColorAttachment()->MakeOGLTexture(texture::type::TEXTURE_2D));
	//CR(m_pOGLFramebuffer->GetColorAttachment()->AttachTextureToFramebuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1));
	//
	//// Albedo (Diffuse)
	//// Alpha channel for specular (not used)
	//CR(m_pOGLFramebuffer->AddColorAttachment());
	//CR(m_pOGLFramebuffer->GetColorAttachment()->MakeOGLTexture(texture::type::TEXTURE_2D));
	//CR(m_pOGLFramebuffer->GetColorAttachment()->AttachTextureToFramebuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2));
...
RESULT OGLProgramGBuffer::OGLInitialize(version versionOGL) {
	RESULT r = R_PASS;
	...
	// Vertex
	CRM(MakeVertexShader(L"gbuffer.vert"), "Failed to create vertex shader");

	// Fragment
	CRM(MakeFragmentShader(L"gbuffer.frag"), "Failed to create fragment shader");
...
RESULT OGLProgramGBuffer::SetupConnections() {
	RESULT r = R_PASS;
...
RESULT OGLProgramGBuffer::ProcessNode(long frameID) {
	RESULT r = R_PASS;
...
RESULT OGLProgramGBuffer::SetObjectTextures(OGLObj *pOGLObj) {
	return R_NOT_IMPLEMENTED;
}

RESULT OGLProgramGBuffer::SetObjectUniforms(DimObj *pDimObj) {
...
RESULT OGLProgramGBuffer::SetMaterial(material *pMaterial) {
	RESULT r = R_PASS;
...
RESULT OGLProgramGBuffer::SetCameraUniforms(camera *pCamera) {
...
RESULT OGLProgramGBuffer::SetCameraUniforms(stereocamera* pStereoCamera, EYE_TYPE eye) {
