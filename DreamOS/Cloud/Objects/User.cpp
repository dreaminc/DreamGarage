...
bool User::LoadProfile()
{
	...
	std::string auth = "Authorization: JWT " + m_token;
	
	auto headers = Http::JsonHeader();
	headers.push_back(auth);

	bool res = Http::GetHttp()->GET("http://10.0.75.1:8000/user/",
		headers,
		resp);
	...
	m_profile = {
		...
		jsonResponse["/data/screen_name"_json_pointer].get<std::string>(),
